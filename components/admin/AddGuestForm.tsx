...
const inputCls = 'border border-black/[0.12] rounded-lg px-2.5 py-2 text-sm font-sans bg-white focus:outline-none focus:ring-1 focus:ring-black/20 focus:border-black/25 transition-colors'
...
export default function AddGuestForm({ onAdded }: Props) {
  ...
  return (
    <div className="bg-white border border-black/[0.07] rounded-2xl shadow-sm p-5 mb-6">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col md:flex-row gap-2 md:items-end">
        ...
        <div className="flex gap-2 flex-1">
          <div className="flex-1">
            <label className="text-xs text-charcoal/70 mb-1 block">שם *</label>
            <input
              type="text"
              value={name}
          ...
          <div className="w-36 md:w-52 shrink-0">
            <label className="text-xs text-charcoal/70 mb-1 block">טלפון</label>
            <input
              type="tel"
              dir="ltr"
              value={phone}
        ...
        <div className="flex gap-2 items-end">
          ...
          <div className="shrink-0">
            <label className="text-xs text-charcoal/70 mb-1 block text-center">מבוגרים</label>
          ...
          <div className="shrink-0">
            <label className="text-xs text-charcoal/70 mb-1 block text-center">ילדים</label>
            <div className="flex items-center gap-1.5 h-9">
              <button
                ...
                className={[
                  ...
                  childrenAllowed ? 'bg-charcoal' : 'bg-black/[0.10] border border-charcoal/20',
                ].join(' ')}
          ...
          <button
            type="submit"
            disabled={loading}
            className="shrink-0 btn-primary px-4 py-2 h-9 whitespace-nowrap"
          >
            {loading ? '...' : '+ הוסף'}
          </button>
        </div>
      ...
      {lastGuest && (
        <div className="mt-3 flex items-center gap-2 bg-black/[0.03] border border-black/[0.08] rounded-xl p-2.5">
          <p className="text-xs text-charcoal/70 flex-1 font-mono truncate" dir="ltr">
          ...
          <div className="flex gap-1.5">
            ...
            <button onClick={copyLink}
              className="text-xs bg-white border border-black/[0.10] px-2.5 py-1 rounded-lg hover:bg-black/[0.04] transition whitespace-nowrap select-none">
              {copied ? '✓' : 'העתק'}
            </button>
          </div>
        </div>
