...
export default function CsvUpload({ onImported }: Props) {
  ...
  return (
    <div className="bg-white border border-black/[0.07] rounded-2xl shadow-sm p-6 mb-6">
      <h2 className="font-serif text-lg font-normal text-charcoal mb-1">ייבוא CSV</h2>
      <p className="text-xs text-charcoal/65 font-sans mb-4 tracking-wide">
      ...
      <div
        ...
        className={[
          ...
          dragging
            ...
            : 'border-black/[0.12] hover:border-black/25 hover:bg-black/[0.03]',
        ].join(' ')}
      >
        ...
        {preview ? (
          <div className="flex flex-col items-center gap-1">
            ...
            <p className="text-xs text-charcoal/60 font-sans">לחץ להחלפה</p>
          </div>
        ) : (
      ...
      {preview && (
        <>
          <div className="mt-3 text-xs text-charcoal/70 font-sans bg-black/[0.03] rounded-xl border border-black/[0.08] p-3 max-h-32 overflow-y-auto space-y-0.5">
            {preview.slice(0, 8).map((r, i) => (
              <div key={i} className="flex gap-2 items-center">
                <span className="font-medium text-charcoal/80">{r.name}</span>
                <span className="text-charcoal/50">·</span>
                <span>{r.pax} אורחים</span>
                {r.phone && <><span className="text-charcoal/50">·</span><span dir="ltr">{r.phone}</span></>}
                {r.childrenAllowed && (
                  <>
                    <span className="text-charcoal/50">·</span>
            ...
            {preview.length > 8 && <p className="text-charcoal/55 pt-1">ועוד {preview.length - 8}...</p>}
          </div>
          <button
